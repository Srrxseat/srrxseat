...
- **ขนาด:** **กว้างคงที่ 1500px** (default) — ทุกภาพได้ watermark ขนาด**พิกเซลเท่ากันเป๊ะ** ไม่ว่าจะเป็น landscape / portrait / square หรือ dimension ต้นฉบับต่างกันแค่ไหน
- **Opacity:** 99%
...
| `SRRXSEAT_OPACITY` | `0.99` | ความโปร่งใส 0.0–1.0 |
| `SRRXSEAT_SUFFIX` | `_wm` | คำต่อท้ายชื่อไฟล์ผลลัพธ์ |
| `SRRXSEAT_WIDTH` | `1500` | ขนาด watermark — ดูตารางด้านล่าง |

**รูปแบบที่รับได้สำหรับ `SRRXSEAT_WIDTH`:**

| ค่า | ความหมาย |
| --- | --- |
| `1500` หรือ `1500px` | พิกเซลคงที่ (default) — แนะนำ ขนาดเท่ากันทุกภาพ |
| `90%` | 90% ของ**ด้านสั้นที่สุด** (เปลี่ยนตามภาพ) |
| `90%l` | 90% ของด้านยาวที่สุด |
| `90%w` | 90% ของกว้างภาพ |
| `90%h` | 90% ของสูงภาพ |

ปรับขนาด watermark ถาวร: แก้ไฟล์ `~/.srrxseat/apply_watermark.sh` บรรทัด `WIDTH_SPEC="${SRRXSEAT_WIDTH:-1500}"` เปลี่ยน `1500` เป็นค่าที่ต้องการ เช่น `1200px`, `1800px`, หรือ `90%`
...
```
magick INPUT \
  \( WATERMARK -resize 1500x -alpha set -channel A -evaluate multiply 0.99 +channel \) \
  -gravity center -compose over -composite \
  OUTPUT
```

- `-resize 1500x` ย่อ/ขยาย watermark เป็นกว้างคงที่ 1500px (คงสัดส่วน) — ผลลัพธ์ขนาดพิกเซลเท่ากันทุกภาพ
- `-evaluate multiply 0.99` ปรับ alpha channel เป็น 99%
